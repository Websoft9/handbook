...
**验收标准：**
- [ ] 安装 `@easyops-cn/docusaurus-search-local` 最新稳定版本 (v0.52.2+)
- [ ] 在 `docusaurus.config.js` 中正确配置主题
- [ ] 启用中英文双语搜索支持 (`language: ["en", "zh"]`)
- [ ] 索引所有 docs 文档，不索引 blog，启用哈希缓存
- [ ] 搜索功能验证：搜索框、快捷键、中英文搜索、结果高亮
- [ ] 开发服务器和生产构建均成功
- [ ] 搜索索引文件大小合理 (<2MB)
...
**验收标准：**
- [ ] 实现 Markdown 语法检查（markdownlint 或类似工具）
- [ ] 验证所有内部链接的正确性（相对路径、锚点链接）
- [ ] 检查外部链接的可达性（可选但推荐）
- [ ] 确保文档与 Docusaurus 编译兼容（MDX、Mermaid、frontmatter）
- [ ] 集成到 CI/CD 工作流（npm 脚本、pre-commit hook 或 GitHub Actions）
- [ ] 提供清晰的使用说明和配置文档
...
**验收标准：**
- [ ] 在 `docusaurus.config.js` 中配置 i18n：默认 `zh`，支持 `['zh', 'en']`
- [ ] 导航栏添加语言下拉切换器
- [ ] 创建 `i18n/en/` 目录结构
- [ ] 生成并翻译主题和插件的 JSON 文件
- [ ] 至少翻译 1-2 个示例文档到英文
...
**验收标准：**
- [ ] 提供核心开发命令（install, dev/start, build, serve, clean）
- [ ] 集成文档验证命令（validate, validate-md, validate-links, validate-quick）
- [ ] 支持 BMad Method 工作流（bmad-install, bmad-status, help-bmad）
- [ ] 提供部署命令（deploy）
- [ ] 实现工具命令（help, check, version）
- [ ] 提供清晰的帮助系统（`make help` 显示所有命令及说明）
...
**验收标准：**
- [ ] 创建 `.github/workflows/ci.yml` - PR 触发 CI 检查
- [ ] CI 调用 `make validate` 运行所有验证（Markdown 语法、链接检查、构建测试）
- [ ] CI 状态检查必须通过才允许合并到 main 分支
- [ ] CD 使用 `make build` 生成静态文件并部署到 Cloudflare Pages（基于 `.github/workflows/cd.yml`）
- [ ] 工作流运行时间合理（CI < 3分钟）
- [ ] 失败时提供清晰的错误信息
